@section('footer')
    <footer class="footer">
        <div class="footer__footer-content">
            @if(!empty($gameInFooter))
                <div class="random-product-container">
                    <div class="random-product-container__head">Случайный товар</div>
                    <div class="random-product-container__content">
                        <div class="item-product">
                            <div class="item-product__title-product">
                                <a href="/games/{{$gameInFooter[0]['id']}}" class="item-product__title-product__link">
                                    {{$gameInFooter[0]['name']}}
                                </a>
                            </div>
                            <div class="item-product__thumbnail">
                                <a href="/games/{{$gameInFooter[0]['id']}}" class="item-product__thumbnail__link">
                                    <img src="/img/cover/{{$gameInFooter[0]['photo']}}" alt="Preview-image" class="item-product__thumbnail__link__img">
                                </a>
                            </div>
                            <div class="item-product__description">
                                <div class="item-product__description__products-price">
                                    <span class="products-price">
                                        {{$gameInFooter[0]['purchase_price']}} руб
                                    </span>
                                </div>
                                <div class="item-product__description__btn-block">
                                    <a href="/games/{{$gameInFooter[0]['id']}}" class="btn btn-blue">
                                        Купить
                                    </a>
                                </div>
                            </div>
